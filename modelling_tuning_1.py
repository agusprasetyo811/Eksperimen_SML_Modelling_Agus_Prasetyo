...
class Config:
    ...
    def __init__(self):
        ...
        # Enhanced optimization settings (INCREASED FOR BETTER ACCURACY)
        self.n_iter_random = 100        # Increased from 30
        self.n_iter_optuna = 200        # Increased from 50  
        self.grid_search_iter = 200     # Increased from 100
        
        # Target improvement
        self.target_improvement = 0.025
        
        # ROBUST SETTINGS - Added untuk prevent overly conservative models
        self.min_approval_rate = 0.3  # Minimum 30% approval rate
        self.max_approval_rate = 0.8  # Maximum 80% approval rate
        self.target_approval_rate = 0.6  # Target ~60% approval rate
        ...
        # MLflow settings
        self.experiment_name = "credit-approval-prediction"
        self.model_name = os.getenv('MODEL_NAME', 'credit_approval_model')
        
        # DagsHub settings (optional)
...
def optimize_multiple_models(data, config, logger, mlflow=None):
    """Optimize multiple model types dengan business constraints"""
...
def save_best_model(best_model_result, config, logger, mlflow=None):
    ...
    try:
        ...
        # Save model
        # model_filename = f"{config.output_dir}/best_{config.model_name}_{timestamp}.pkl"
